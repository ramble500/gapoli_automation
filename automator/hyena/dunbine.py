...
def main(c: Controller, hall: str):
    ...
    for i in range(10):
        ...
        for i, seat in enumerate(seats_info):
            ...
            if os.path.exists(f"log/hyena/graph_dunbine/True/{graph_hash}.json"):
                logger.info(
                    f"---------------------------------Exists True {graph_hash}---------------------------------"
                )
            if os.path.exists(f"log/hyena/graph_dunbine/False/{graph_hash}.json"):
                logger.info(
            ...
            if True:
                ...
                is_purple = judge_ss(ss)
                logger.info(f"is_purple={is_purple}")

                path = f"log/hyena/ss_dunbine/{is_purple}/{graph_hash}_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}.png"
                os.makedirs(os.path.dirname(path), exist_ok=True)
                c.save_ss(path, ss)

                path = f"log/hyena/graph_dunbine/{is_purple}/{graph_hash}.json"
                os.makedirs(os.path.dirname(path), exist_ok=True)
...
color_crop = RelativeCrop(
    xl=0.45357142857142857,
    xr=0.5410714285714285,
    yl=0.553921568627451,
    yr=0.5943627450980392,
)


def judge_ss(ss):
    crop_ss = color_crop.crop_pil(ss)
    mean_color = np.array(crop_ss).mean(axis=0).mean(axis=0) / 256
    return 0.7 < colorsys.rgb_to_hsv(*mean_color)[0] < 0.9
